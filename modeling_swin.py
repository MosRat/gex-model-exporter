...
def window_partition(input_feature, window_size):
    ...
    windows = (
        ...
        .contiguous()
        .view(-1, int(window_size), int(window_size), int(num_channels))
    )
    return windows
...
class SwinPatchEmbeddings(nn.Module):
    ...
    def forward(
        ...
        print(f"Pix shape {pixel_values.shape}")
        _, num_channels, height, width = pixel_values.shape
        # pad the input to be divisible by self.patch_size, if needed
        pixel_values = self.maybe_pad(pixel_values, height, width)
        embeddings = self.projection(pixel_values)
        _, dim, height, width = embeddings.shape
        height = int(height)
        width = int(width)
        dim = int(dim)
        output_dimensions = (height, width)
        print(embeddings.shape)
        embeddings = embeddings.view(-1, dim, height * width).transpose(1, 2)
...
class FusedSelfAttention(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        x: torch.Tensor,
        ...
        q, k, v = (
            i.unflatten(-1, (num_head, -1)).transpose(1, 2)
            for i in torch.split(x @ qkv_w + qkv_b, qkv_w.shape[1] // 3, -1)
        )
    ...
    def symbolic(
        g: GraphContext,
        x: torch._C.Value,
        qkv_w: torch._C.Value,
        qkv_b: torch._C.Value,
        attn_bias: torch._C.Value,
        num_heads: int,
    ):
        output: torch._C.Value = g.op(
...
# def fused_attn(x: FLOAT,qkv:FLOAT,num_head: int,attn_bias:FLOAT) -> FLOAT:
#     return microsoft_op.Attention()
...
class SwinSelfAttention(nn.Module):
    def __init__(self, config, dim, num_heads, window_size):
        ...
        # print(window_size, relative_position_index.shape, self.relative_position_bias_table.shape)
    ...
    def convert_qkv(self):
        self.qkv = nn.Linear(self.all_head_size, self.all_head_size * 3, bias=True)
        self.qkv.weight.data = torch.concat(
            [self.query.weight.data, self.key.weight.data, self.value.weight.data],
            dim=0,
        )
        self.qkv.bias.data = torch.concat(
            [self.query.bias.data, self.key.bias.data, self.value.bias.data], dim=0
        )
        del self.query
        del self.key
        del self.value
    ...
    def forward(
        ...
        attn_bias = (
            (relative_position_bias.unsqueeze(0) + attention_mask.unsqueeze(1))
            if attention_mask is not None
            else relative_position_bias.unsqueeze(0)
        )
        attn_bias = torch.from_numpy(attn_bias.detach().cpu().numpy()).to(
            hidden_states.device
        )
        attn_output = FusedSelfAttention.apply(
            hidden_states,
            qkv_w.transpose(-1, -2),
            qkv_b,
            attn_bias,
        ...
        #         self.transpose_for_scores(i)
        #         for i in torch.split(qkv, qkv.shape[-1] // 3, -1)
        #     )
        #     print(qkv.shape, query_layer.shape)
        # else:
        ...
        # #     head_mask.shape if head_mask is not None else None,
        # # )
        # if attention_mask is not None:
        ...
        # print(f"diff = {torch.nn.functional.mse_loss(attn_output,context_layer)}")
...
class FusedGemmGelu(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        x: torch.Tensor,
        ...
        inter = torch.nn.functional.linear(x, w.transpose(-1, -2), b)
        return torch.nn.functional.gelu(inter)
    ...
    def symbolic(
        ...
        ret =  g.op(
            "Unsqueeze",
            g.op("com.microsoft::Gelu", g.op("Gemm", g.op("Squeeze", x), w, b)),
            torch.tensor(0,dtype=torch.int64),
        )
        # ret = g.op("com.microsoft::Gelu", g.op("Gemm", x, w, b))
        # return g.op(
        #     "Unsqueeze",
        #     g.op("com.microsoft::GemmFastGelu", g.op("Squeeze", x), w, b),
        #     torch.tensor(0, dtype=torch.int64),
        # )
        # ret = g.op("com.microsoft::GemmFastGelu", x, w, b)
        t_ = x.type()
        s_ = x.type().sizes()
        s_w = w.type().sizes()
        ret.setType(t_.with_sizes([s_[0], s_[1], s_w[0]]))
        return ret
    
class FusedGeluMLP(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        x: torch.Tensor,
        ...
        inter = torch.nn.functional.linear(x, w1.transpose(-1, -2), b1)
        inter = torch.nn.functional.gelu(inter)
        inter = torch.nn.functional.linear(inter, w2.transpose(-1, -2), b2)
    ...
    def symbolic(
        ...
        ret = g.op(
            ...
            g.op(
                "Gemm",
                g.op("com.microsoft::Gelu", g.op("Gemm", g.op("Squeeze", x), w1, b1)),
                w2,b2,
            ),
            torch.tensor(0, dtype=torch.int64),
        )
...
class SwinIntermediate(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # return hidden_states
        return FusedGemmGelu.apply(
            hidden_states, self.dense.weight.transpose(-1, -2), self.dense.bias
        )
...
class SwinLayer(nn.Module):
    ...
    def get_attn_mask(self, height, width, dtype, device):
        if self.shift_size > 0:
            ...
            attn_mask = torch.from_numpy(attn_mask.detach().cpu().numpy()).to(
    ...
    def maybe_pad(self, hidden_states, height, width):
        ...
        # print(hidden_states.shape,pad_values)
    ...
    def forward(
        ...
        #     head_mask,
        # )
        # print(always_partition)
        if hasattr(self, "input_dimensions"):
            input_dimensions = self.input_dimensions
        else:
            self.input_dimensions = input_dimensions
            
        if not always_partition:  # False
        ...
        hidden_states = hidden_states.view(
            int(batch_size), height, width, int(channels)
        )

        # pad hidden_states to multiples of window size
        # hidden_states, height_pad = self.maybe_pad(hidden_states, height, width)
        height_pad = int(height)
        width_pad = int(width)
        ...
        hidden_states_windows = hidden_states_windows.view(
            -1, self.window_size * self.window_size, int(channels)
        )
        ...
        attention_windows = attention_windows.view(
            int(batch_size), int(height * width), int(channels)
        )
        ...
        layer_output = FusedGeluMLP.apply(
            layer_output,
            self.intermediate.dense.weight.transpose(-1, -2),
            self.intermediate.dense.bias,
            self.output.dense.weight.transpose(-1, -2),
            self.output.dense.bias,
        )
...
class SwinStage(nn.Module):
    ...
    def forward(
        ...
        if init:
            stage_outputs += (output_dimensions,)
